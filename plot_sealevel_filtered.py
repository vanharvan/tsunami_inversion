...
# --- NEW: FILTER TIDE (Rolling Mean Method) ---
# We do this BEFORE slicing to maintain accuracy at the start/end of the window.
...
end_time = eq_origin + pd.Timedelta(hours=1)

# ---- ARRIVAL PICKING ------
arrival_time = pd.to_datetime("2025-07-29 23:28:30", utc=True)
...
# Plot 1: Original vs Tide
#plt.subplot(2, 1, 1)
#plt.plot(df_plot["time_utc"], df_plot["sealevel_m"], color='lightgray', label="Observed Data", linewidth=1)
#plt.plot(df_plot["time_utc"], df_plot["tide_background"], color='blue', linestyle='--', label="Estimated Tide", linewidth=1.5)
#plt.axvline(x=eq_origin, color='red', linestyle='-', linewidth=2, label="EQ Origin")
#plt.title(f"Original Data & Tide - {station_code}")
#plt.ylabel("Sea Level (m)")
#plt.legend(loc='upper right')
#plt.grid(True, alpha=0.5)

# Plot 2: Tsunami Only (Residual)
#plt.subplot(2, 1, 2)
